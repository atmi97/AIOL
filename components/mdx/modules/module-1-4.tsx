...
const CLASSES = [
  ...
  { k: "internal", name: "Internal", example: "Internal process docs · non-PII meeting notes · drafts · aggregate analytics", rule: "Only approved AI tools on the register.", caveat: "Prohibited in public ChatGPT, consumer Claude, Gemini consumer, or personal-account Copilot.", color: "brand" },
  { k: "confidential", name: "Confidential · PHI · PII", example: "Employee records · payroll · benefit claims · EFAP notes · HR investigations · SINs", rule: "Must not go in external tools. PHI requires de-identification unless explicit, documented consent.", caveat: "Even in approved tools: data minimization — don't include more PII than the task actually requires.", color: "amber" },
  { k: "restricted", name: "Restricted", example: "Credentials · security tokens · legal hold · privileged comms · critical source code", rule: "Must not be processed in any AI tool.", caveat: "Under active review by the AI Steering Committee for agentic/local-model scenarios. For Tier 1: do not enter anywhere.", color: "rose" },
];
    ...
    <div className="mt-6 space-y-3">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {CLASSES.map((c) => {
          ...
          return (
            <button key={c.k} onClick={() => setActive(c.k)} className={`rounded-xl p-4 text-left border transition bg-white ${tone[c.color]}`}>
...
function ProhibitedGrid() {
  return (
    <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
      {PROHIBITED.map((p) => (
        <div key={p.label} className="rounded-xl border border-rose-200 bg-gradient-to-br from-rose-50 to-white p-4">
  ...
  return (
    <>
      <Section id="laws" n="1.4.1" title="The Saskatchewan privacy landscape">
        ...
        <Tabs
          items={[
            { key: "hipa", label: "HIPA", subtitle: "The central law for health", body: (<><p><b>The Health Information Protection Act</b> (Saskatchewan).</p><p>Protects <b>personal health information</b>. Applies to trustees — SHA, 3sHealth when acting as an information management service provider, physicians, pharmacists, hospitals.</p><Tag tone="amber">Most restrictive. PHI gets the highest bar in AI contexts.</Tag></>) },
            { key: "lafoip", label: "LA FOIP", subtitle: "Local authorities", body: (<><p><b>The Local Authority Freedom of Information and Protection of Privacy Act</b> (Sask.).</p><p>Protects personal information held by local authorities — regional health authorities, schools, municipalities. SHA is a local authority under LA FOIP.</p></>) },
            { key: "foip", label: "FOIP", subtitle: "Provincial government", body: (<><p><b>The Freedom of Information and Protection of Privacy Act</b> (Sask.).</p><p>Protects personal information held by the provincial government. Some 3sHealth work touches FOIP through ministry partners.</p></>) },
            { key: "pipeda", label: "PIPEDA", subtitle: "Federal · commercial", body: (<><p><b>Personal Information Protection and Electronic Documents Act</b> (Federal).</p><p>Applies to commercial activity. Because Saskatchewan doesn't have substantially similar private-sector legislation, PIPEDA applies to commercial activity within the province not covered elsewhere.</p></>) },
          ]}
        />
      ...
      <Section id="classification" n="1.4.3" title="The AUP data classification">
        <p>The AUP sorts data into four classifications. Tier 1 teaches the classifications; Tier 2 Module 2.4 drills application in harder, ambiguous scenarios.</p>
        <ClassificationExplorer />
      </Section>

      <Section id="prohibited" n="1.4.4" title="Data explicitly prohibited">
        <p>AUP Table 2 lists categories of data that are <b>explicitly prohibited</b> from AI tool input. Tier 1 learners are expected to recognize these on sight.</p>
        <ProhibitedGrid />
      ...
      <Section id="approved" n="1.4.5" title="Approved versus unapproved tools">
        ...
        <TwoColCompare
          left={{ title: "Approved", lead: "On the register · evaluated · tenant-bounded", points: ["Tenanted M365 Copilot (3sHealth work account)", "Tools evaluated against privacy & security requirements", "Residency specified on the register", "Classification & tier requirements documented"], tone: "emerald" }}
          right={{ title: "Unapproved", lead: "Not on the register — regardless of vendor", points: ["ChatGPT on a personal account", "Claude on a browser tab without enterprise context", "Gemini through a personal Google account", "Personal-account Copilot (even though tenanted Copilot is approved)"], tone: "rose" }}
        />
      ...
      <Section id="tenant" n="1.4.7" title="Tenant boundaries and data residency">
        <p>Two technical ideas worth a few minutes — they come up constantly in AI conversations.</p>
        <TwoColCompare
          left={{ title: "Tenant boundary", lead: "The slice of a cloud service that belongs to you", points: ["3sHealth M365 tenant ≠ your personal Microsoft account", "Within your tenant: AI sees what you can see, stays inside", "Personal sign-in: outside the tenant, different model, different terms", "Enforced by sign-in context — not vendor reputation"], tone: "brand" }}
          right={{ title: "Data residency", lead: "Where, geographically, data is stored & processed", points: ["Canadian public/health sectors favour Canadian or non-commercial-US", "Register specifies residency for each approved tool", "Unapproved tool → residency unknown", "Unknown residency is itself a reason for prohibition above Public"], tone: "slate" }}
        />
      </Section>
      ...
      <Section id="activity" n="1.4.8" title='Activity: "Can you enter this?"'>
        ...
        <ClassifyQuiz
          ...
          items={[
            { q: "You paste a published 3sHealth job posting into public web ChatGPT to rewrite it for clarity.", a: "prohibit", why: "Content is Public, but the tool is unapproved. Using unapproved tools is Shadow AI — regardless of data classification. Use an approved alternative." },
            { q: "You use tenanted M365 Copilot in Outlook to draft a polite follow-up to a vendor whose RFP response you have. Vendor name and contract type are in your prompt.", a: "caution", why: "Tool is approved and tenanted. Data is Internal. Acceptable — avoid anything that drifts into Confidential." },
            { q: "You copy an employee's EFAP case summary into tenanted Copilot in Word to rewrite it in plain language for a managerial overview.", a: "prohibit", why: "EFAP case data is PHI. Prohibited in any AI tool unless explicit consent is obtained and documented (AUP Section 4 + HIPA)." },
            { q: "You ask tenanted Copilot to summarize a 60-page draft policy marked Internal, inside the 3sHealth tenant.", a: "permit", why: "Approved tool, appropriate classification. Standard use." },
            { q: "You paste a section of PL/SQL — including a database connection string — into an unapproved AI coding assistant to ask for a refactor.", a: "prohibit", why: "Two violations: unapproved tool AND the data includes a credential (restricted under AUP Table 2). This is Shadow AI plus credentials exposure." },
          ]}
        />
      </Section>
