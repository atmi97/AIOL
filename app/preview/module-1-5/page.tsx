    ...
    <div className="mt-6 grid grid-cols-1 md:grid-cols-[1fr_auto_1fr_auto_1fr] items-stretch gap-3">
      <div className="rounded-xl border-2 border-brand-300 bg-gradient-to-br from-brand-50 to-white p-5">
        <div className="text-[10px] font-semibold uppercase tracking-wider text-brand-700">
          Step 1 · Human at the start
        </div>
        <div className="mt-2 text-lg font-bold text-slate-900">You define the task</div>
        <div className="mt-2 text-xs text-slate-600 leading-relaxed">
          Choose the tool, supply the prompt, decide what data is appropriate.
        </div>
      </div>
      ...
      <div className="rounded-xl border-2 border-brand-300 bg-gradient-to-br from-brand-50 to-white p-5">
        <div className="text-[10px] font-semibold uppercase tracking-wider text-brand-700">
          Step 3 · Human at the end
        </div>
        <div className="mt-2 text-lg font-bold text-slate-900">You review & own it</div>
        <div className="mt-2 text-xs text-slate-600 leading-relaxed">
          Correct what needs correcting. Take accountability for the final version.
        </div>
      </div>
    </div>
  );
  ...
  const steps = [
    {
      ...
      when: "Immediately for anything material. Your manager has context on the task and the stakeholder relationship.",
    },
    {
      n: "02",
      who: "Manager → IT Security / ESS",
      when: "For data concerns, potential breach, credentials exposure, or tool misbehaviour.",
    },
    {
      n: "03",
      who: "ESS → AIGC",
      when: "For anything that rises to policy, pattern, or cross-organizational concern.",
    },
  ];
  return (
      ...
      <div className="space-y-3">
        {steps.map((s) => (
          <div key={s.n} className="flex gap-4">
            <div className="shrink-0 w-10 h-10 rounded-full bg-brand-600 text-white font-bold flex items-center justify-center text-sm shadow-md relative z-10">
              {s.n}
            </div>
      ...
      {items.map((h) => (
        <div key={h.t} className="rounded-xl border border-slate-200 bg-white p-4 hover:border-brand-300 hover:shadow-md transition">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="px-2 py-0.5 rounded-full bg-brand-50 text-brand-700 text-[10px] font-medium border border-brand-100">
              {h.badge}
            </span>
          </div>
...
function ScenarioWalkthrough() {
  ...
  const steps = [
    ...
    },
    {
      ...
      color: "amber",
      actions: [
        "File a ServiceNow AI incident under the AI Incident Category. Near-miss classification.",
        "Include: tool (tenanted M365 Copilot in Word), classification (Internal, no PHI), nature of error (fabricated figure, plausible magnitude), and that it was caught pre-send.",
        "Notify your manager. They may want to review the final draft before it goes out.",
      ],
    },
    {
      label: "By tomorrow morning",
      color: "brand",
      actions: [
        "Final review with a second pair of eyes for the client-facing version. Not required by policy — good practice given the catch.",
  ...
  const toneBorder: Record<string, string> = {
    rose: "border-rose-300 bg-gradient-to-br from-rose-50 to-white",
    amber: "border-amber-300 bg-gradient-to-br from-amber-50 to-white",
    brand: "border-brand-300 bg-gradient-to-br from-brand-50 to-white",
  };
  ...
  return (
    <div className="mt-5 space-y-4">
        ...
        {steps.map((s, i) => (
          <button
            ...
            className={`rounded-xl p-3 border text-left transition ${
              step === i
                ? toneBorder[s.color] + " shadow-md"
                : "border-slate-200 bg-white hover:border-brand-300"
            }`}
          >
            <div className={`text-[10px] font-semibold uppercase tracking-wider ${step === i ? toneLabel[s.color] : "text-slate-500"}`}>
              Step {i + 1}
            </div>
      ...
      <div className={`rounded-xl border-2 p-5 ${toneBorder[active.color]}`}>
        <div className={`text-xs font-semibold uppercase tracking-wider ${toneLabel[active.color]}`}>
          {active.label}
        </div>
        <ul className="mt-3 space-y-2">
          {active.actions.map((a, i) => (
            <li key={i} className="flex gap-3 text-sm text-slate-800">
              <span className="shrink-0 w-5 h-5 rounded-full bg-white border border-slate-300 text-slate-600 text-[11px] font-semibold flex items-center justify-center mt-0.5">
                {i + 1}
              </span>
              <span>{a}</span>
            </li>
  ...
  return (
    ...
    >
      <Section id="accountability" n="1.5.1" title="The human accountability model">
        ...
        <BookendFlow />
        <KeyMessage>
          This applies equally to a five-second Copilot prompt and a multi-hour agentic workflow. The bookends are not optional.
        </KeyMessage>
      </Section>
      ...
      <Section id="incidents" n="1.5.2" title="What counts as an AI incident">
        ...
        <Accordion
          ...
          items={[
            {
              title: "Output error that reached a stakeholder",
              subtitle: "Material error sent to employee, vendor, client, or regulator",
              body: '"Material" is a judgment call — the bar is much lower for PHI, financial, and legal content than for a typo.',
            },
            {
              title: "Data concern or possible breach",
              subtitle: "PHI pasted into an unapproved tool · credentials in a prompt · retention concerns",
              body: "Report even if you're not sure. The incident team can help determine whether it rises to a breach.",
            },
            {
              title: "Bias detected in output",
              subtitle: "Systematic skew in hiring, scoring, routing, review language",
              body: "Worth reporting even if no decision was made on the biased output. The pattern matters.",
            },
            {
              title: "Observed Shadow AI",
              subtitle: "A colleague using an unapproved tool for 3sHealth work",
              body: "Report it. The report goes to the process, not the person — the system is designed to catch exposure before it becomes a breach.",
            },
            {
              title: "Tool behaviour suggesting a security incident",
              subtitle: "Unusual access · unexpected data surfacing · apparent guardrail bypass",
              body: "Goes through the standard IT Security path with an AI tag.",
            },
            {
              title: "Near-misses",
              subtitle: "You caught the error before it went out",
              body: "Still report it. Near-miss reporting is how the organization improves training, updates the prompt library, and spots patterns. Good-faith reporting is encouraged.",
            },
          ]}
        />
      </Section>
      ...
      <Section id="how" n="1.5.3" title="How to report">
        <p>
          Reports go through <b>ServiceNow</b>, under the <Tag>AI Incident Category</Tag>. Tier 2 Module 2.7 walks through the exact screens; for Tier 1 the key information is <i>what to include</i>.
        </p>
      ...
      <Section id="culture" n="1.5.6" title="A note on culture">
        <p>
          This program does not treat AI incidents as failures to be punished. It treats them as <b>signals</b> that help the organization learn.
        </p>
        <KeyMessage>
